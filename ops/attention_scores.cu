...
__global__ void attention_scores_kernel(QKVHeadView view, float* scores) {
    ...
    if(q_token >= view.seq_len || k_token >= view.seq_len) {return;}
    float score = 0.0f;

    for(int d = 0; d < view.head_dim; d++) {
        float q = view.q[q_token * view.row_stride + d];
        float k =view.k[k_token * view.row_stride + d];
        score += q * k;
    }
...
void launch_attention_scores(QKVHeadView view, float* scores) {
    dim3 block(TILE, TILE);
    dim3 grid((view.seq_len + TILE - 1) / TILE, (view.seq_len + TILE - 1) / TILE);
